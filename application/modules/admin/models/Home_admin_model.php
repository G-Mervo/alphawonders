...
class Home_admin_model extends CI_Model
{
    ...
    {
        $result = $this->db->query("SELECT YEAR(FROM_UNIXTIME(date)) as year, MONTH(FROM_UNIXTIME(date)) as month, COUNT(id) as num FROM orders ORDER BY YEAR(FROM_UNIXTIME(date)), MONTH(FROM_UNIXTIME(date)) ");
    ...
    public function getValueStore($key)
    {
        ...
        // $img = $query->row_array();
        // var_dump($img); echo "<br>";
        
        // return $img['value'];
        // var_dump($img['value']); exit;
    }
